...
EditorLayer::EditorLayer()
  : m_EditorCamera(0.1f, 100.f, glm::radians(90.f), 0.f,
                   glm::vec3(0.f, 0.f, 0.f), glm::vec3(0.f, 0.f, 0.f))
{
  VRM_ASSERT_MSG(INSTANCE == nullptr,
                 "Only one instance of EditorLayer is allowed");
  INSTANCE = this;
...
EditorLayer& EditorLayer::Get() { return *INSTANCE; }

void EditorLayer::loadScene(const std::string&       name,
                            std::unique_ptr<Scene>&& scene)
{
  ...
  {
    RenderPipeline& oldPipeline =
      gameLayer.getScene().getRenderer().getRenderPipeline();
    ...
    scene->getRenderer().setFrameSize(
      { m_lastViewportInfos.width, m_lastViewportInfos.height });
...
void EditorLayer::loadScene(const std::string& sceneAssetName)
{
  ...
  scene->loadFromAsset(
    AssetManager::Get().getAsset<SceneAsset>(sceneAssetName));
  loadScene(sceneAssetName, std::move(scene));
}

void EditorLayer::unloadScene(bool force)
{
  if (isSceneLoaded() || force) { loadScene<Scene>(""); }
}
...
void EditorLayer::reloadScripts() { VRM_LOG_WARN("Not implemented"); }
...
void EditorLayer::onInit()
{
  ...
  m_CustomEventManager.bindPermanentCallback(
  ...
  m_TriggerManager.bindPermanentCallback(
    "MoveForward",
    [this](bool triggered)
    {
...
void EditorLayer::onUpdate(const DeltaTime& dt)
{
  ...
  if (cameraInputs) { m_EditorCamera.onUpdate(dt); }
  else
  {
  ...
  const bool lastUpdt =
    (lastSimul || lastPlaying) && !m_lastViewportInfos.paused;

  if (updt != lastUpdt) { app.getGameLayer().setShouldUpdate(updt); }

  if (playing != lastPlaying)
  {
    if (playing) { Application::Get().getGameLayer().getScene().spawn(); }
    else
    {
      // Reloading scene
      if (isSceneLoaded()) loadScene(m_loadedScene);
    }
  }
...
void EditorLayer::_loadScriptsRuntimeLibrary()
{
  ...
  if (g_scriptLibraryPath.size() > 0)
  {
    ...
    {
      VRM_LOG_ERROR("Error while loading scripts runtime library {}",
                    g_scriptLibraryPath);
    }
  }
  else
  ...
  if (ok)
  {
    VRM_LOG_INFO("Scripts runtime library {} loaded",
                 g_scriptLibraryPath.data());
  }
}
