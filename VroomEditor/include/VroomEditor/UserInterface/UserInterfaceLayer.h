...
#define VRM_EDITOR_UI_ELEMENT(Class)                                    \
  (static_cast<Class&>(                                                 \
    UserInterfaceLayer::Get().getElement(EInterfaceElement::e##Class)))
...
public:
  ...
  inline static const std::filesystem::path s_imguiStyleFile =
    "Resources/Editor/Appearance/Style/imguiStyle.json";
...
public:
  ...
  inline void        setViewportInfos(const ViewportInfos& viewportInfo)
  {
    m_ViewportInfo = viewportInfo;
  }

  inline void setElementVisible(EInterfaceElement::Element element,
                                bool                       visible)
  {
    m_openFlags.at(element) = visible;
  }
  inline bool isElementVisible(EInterfaceElement::Element element) const
  {
    return m_openFlags.at(element);
  }

  void saveImguiStyle(const ImGuiStyle& style) const;

  inline ImGuiElement& getElement(EInterfaceElement::Element elem)
  {
    return *m_elements.at(elem);
  }
...
private:
  ...
  T& emplaceImGuiElement(EInterfaceElement::Element elem, Args&&... args,
                         bool                       open = true)
  {
    auto& uniquePtr = m_elements.at(elem) =
...
private:
  ...
  std::array<std::unique_ptr<ImGuiElement>, EInterfaceElement::eElementCount>
                                                     m_elements = {};
