...
public:
  ...
  inline std::span<float const> getMin() const
  {
    return std::span(m_min.data(), m_dim);
  }
  inline std::span<float const> getMax() const
  {
    return std::span(m_max.data(), m_dim);
  }
  inline bool isColorType() const
  {
    return m_type == EScalarType::eColor3 || m_type == EScalarType::eColor4;
  }
