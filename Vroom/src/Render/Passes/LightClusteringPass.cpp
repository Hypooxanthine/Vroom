...
size_t LightClusteringPass::getPerViewSize() const
{
  return sizeof(RawHeader)
       + clusterCount.x * clusterCount.y * clusterCount.z * sizeof(RawCluster);
}

void LightClusteringPass::onInit()
{
  VRM_CHECK_RET_MSG(glm::all(glm::greaterThan(clusterCount, glm::uvec3(0))),
                    "At least one cluster is needed for each dimension");
  ...
  m_setupClustersMat =
    &getPassMaterial(AssetManager::Get().getAsset<MaterialAsset>(
      "Resources/Engine/Material/BuildClustersMaterial.json"));
  m_cullLightsMat =
    &getPassMaterial(AssetManager::Get().getAsset<MaterialAsset>(
      "Resources/Engine/Material/CullLightsMaterial.json"));

  VRM_CHECK_RET_MSG(m_setupClustersMat != nullptr,
                    "Invalid setup cluster material");
...
{
  VRM_PROFILE_SCOPE("LightClusteringPass::onSetup");

  if (m_totalClusters == 0) return;
  ...
  // One header per view
  // It is the same one for all, but will be easier when binding a subrange of
  // buffer later
  ...
  glm::uvec3 dispatch =
    (clusterCount + s_setupClustersLocalSize - glm::uvec3(1))
    / s_setupClustersLocalSize;
  ...
  for (size_t i = 0; i < ctx.views.size(); ++i)
  {
    ...
    setupClustersShader.setUniformMat4f("u_InvProjection",
                                        glm::inverse(camera.getProjection()));
    setupClustersShader.setStorageBuffer("ClusterInfoBlock",
...
{
  VRM_PROFILE_SCOPE("LightClusteringPass::onRender");
  ...
  for (size_t i = 0; i < ctx.views.size(); ++i)
  {
    ...
    cullLightsShader.setStorageBuffer("ClusterInfoBlock",
                                      m_clustersBuffer.getBuffer(),
                                      sizePerView * i, sizePerView);
