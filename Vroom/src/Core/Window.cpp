...
bool Window::create(const std::string& windowTitle, uint32_t width,
                    uint32_t height)
{
  ...
  if (!m_Handle)
  {
    ...
    VRM_LOG_ERROR(
      "Failed to create window. GLFW error code: {0:x}. Description: {1}.",
      errorCode, errorDesc);
    return false;
  }
...
bool Window::getKeyRepeatEnabled() const { return m_KeyRepeatEnabled; }

bool Window::getCursorVisible() const { return m_CursorVisible; }
...
void Window::setCursorVisible(bool visible)
{
  ...
  // If cursor is visible, we need to update the last mouse position because
  // mouse will suddenly appear at its last visible position
  if (visible)
  {
...
{
  return { static_cast<int>(m_lastMouseX), static_cast<int>(m_lastMouseY) };
}
...
bool Window::hasPendingEvents() const { return !m_EventQueue.empty(); }
...
{
  VRM_PROFILE_SCOPE("Window::swapBuffers");
...
void Window::keyCallback(int key, int scancode, int action, int mods)
{
  ...
  else e.type = Event::Type::KeyReleased;
}
...
void Window::mouseCallback(int button, int action, int mods)
{
  ...
  Event& e    = m_EventQueue.emplace();
  e.mouseCode = code;
  if (action == GLFW_PRESS) e.type = Event::Type::MousePressed;
  else e.type = Event::Type::MouseReleased;
}
...
void Window::mouseEnteredCallback(int entered)
{
  Event& e = m_EventQueue.emplace();
  e.type =
    (entered == GLFW_TRUE ? Event::Type::MouseEntered : Event::Type::MouseLeft);
  glfwGetCursorPos(m_Handle, &m_lastMouseX, &m_lastMouseY);
...
void Window::focusCallback(int focused)
{
  Event& e = m_EventQueue.emplace();
  e.type =
    (focused == GLFW_TRUE ? Event::Type::GainedFocus : Event::Type::LostFocus);
}
...
{
  Event& e = m_EventQueue.emplace();
  e.type   = (entered == GLFW_TRUE ? Event::Type::FileDragEnter
                                   : Event::Type::FileDragLeave);
}
