...
//   m_FrameBuffer.create(static_cast<GLuint>(renderer.getViewportSize().x),
//   static_cast<GLuint>(renderer.getViewportSize().y), aa);
//   m_FrameBuffer.bind();
//   m_FrameBuffer.addColorAttachment(0, 4, glm::vec4{ 0.1f, 0.1f, 0.1f, 1.f });
//   m_FrameBuffer.attachRenderBuffer();
//   VRM_ASSERT_MSG(m_FrameBuffer.validate(), "Could not build GameLayer
//   framebuffer");
// }
...
void GameLayer::onInit()
{
  VRM_DEBUG_ASSERT_MSG(
    m_NextScene != nullptr,
    "Make sure you loaded a scene before running the application.");
...
void GameLayer::onNewFrame() { checkNextScene(); }
...
CustomEventBinder
GameLayer::createCustomEvent(const std::string& customEventName)
{
