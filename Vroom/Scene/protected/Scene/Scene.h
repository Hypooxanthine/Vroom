...
protected:
   ...
   * @brief
   *
   * @tparam CallOnFirst If false, every entity should have a valid parent. If false, this function will try to get
   * first's parent, which can be null if first is root.
   * @tparam Fn signature : bool(Entity& currentEntity, Entity& parentEntity). If it returns false, traversal is aborted
