...
Entity::Entity(entt::entity handle, entt::registry* registry, Scene* scene) : m_Handle(handle), m_Scene(scene)
{}

Entity::Entity(const Entity& other) : m_Handle(other.m_Handle), m_Scene(other.m_Scene)
{}
...
ScriptComponent& Entity::getScriptComponent()
{
  return getComponentInternal<ScriptHandler>().getScript();
}

const ScriptComponent& Entity::getScriptComponent() const
{
  return getComponentInternal<ScriptHandler>().getScript();
}

bool Entity::hasScriptComponent() const
{
...
ScriptComponent& Entity::addScriptComponent(ScriptComponentPtr&& script)
{
  ...
  VRM_ASSERT_MSG(component.hasScript(),
                 "Cannot add an empty script through addScriptComponent. Use ScriptHandler component directly.");
