 ...
 *
 */
class VRM_EVENT_API CustomEventBinder
{
public:
  ...
  /**
   * @brief Bind an input to the custom event. Since no code is provided, all inputs of the given type will trigger the
   ...
   */
  inline CustomEventBinder& bindInput(Event::Type inputType)
  {
    return bindInput(inputType, 0);
  }
   ...
   * @return CustomEventBinder A binder to the same CustomEvent instance.
   */
  inline CustomEventBinder& unbindInput(Event::Type inputType)
  {
    return bindInput(inputType, 0);
  }
