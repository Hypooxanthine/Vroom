...
#include "Event/CustomEventBinder.h"


#define VRM_CUSTOMEVENT_CALLBACK(function) \
 ...
 *
 */
class VRM_EVENT_API CustomEventManager
{
public:
  ...
  /**
   * @brief Binds an input to the custom event. Since no code is provided, all inputs of the given type will trigger the
   * custom event, regardless of their code.
   *
   * @param customEventName Name of the custom event.
   * @param inputType Input type to bind.
   * @return CustomEventBinder Custom event binder to bind inputs and callbacks to the custom event.
   */
  inline CustomEventBinder bindInput(const std::string& customEventName, Event::Type inputType)
  {
    return bindInput(customEventName, inputType, 0);
  }
  ...
  /**
   * @brief Unbind an input from the custom event. Since no code is provided, all inputs of the given type will be
   * unbound.
   *
   * @param customEventName Name of the custom event.
   ...
   * @return CustomEventBinder Custom event binder to bind inputs and callbacks to the custom event.
   */
  inline CustomEventBinder unbindInput(const std::string& customEventName, Event::Type inputType)
  {
    return unbindInput(customEventName, inputType, 0);
  }
...
private:
  ...
  std::unordered_map<Event::Type, std::unordered_map<CodeType, std::unordered_set<std::string>>>
    m_RawEventsToCustomEvents;
};
