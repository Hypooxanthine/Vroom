 ...
 *
 */
class VRM_RASTERIZER_API SSBO430Layout
{
public:
  ...
  struct Attrib
  {
    ...
    inline constexpr size_t getLocation() const
    {
      return location;
    }
    inline constexpr size_t getAlignment() const
    {
      return alignment;
    }
...
public:
  ...
  constexpr size_t getRawSize() const
  {
  ...
  {
    VRM_ASSERT_MSG(dim > 0 && dim <= 4, "Only vectors of dimension 1 to 4 are supported");
...
inline constexpr SSBO430Layout::Attrib SSBO430Layout::_push(size_t elemSize, size_t alignment)
{
