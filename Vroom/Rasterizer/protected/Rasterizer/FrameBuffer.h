...
#include "Rasterizer/RenderBuffer.h"
...
class VRM_RASTERIZER_API FrameBuffer
{
public:
...
public:
  ...
  inline void setColorAttachment(GLuint slot, const Texture& colorTexture, GLuint mipLevel = 0,
                                 const glm::vec4& clearColor = glm::vec4{ 0.f, 0.f, 0.f, 1.f })
  {
    ...
    VRM_ASSERT_MSG(colorTexture.getDescription().sampleCount == m_samples,
                   "Samples must match: framebuffer is set with {} samples, but texture has {}", m_samples,
                   colorTexture.getDescription().sampleCount);
  ...
  inline void setColorAttachment(GLuint slot, const Texture& arrayColorTexture, GLuint layer, GLint mipLevel = 0,
                                 const glm::vec4& clearColor = glm::vec4{ 0.f, 0.f, 0.f, 1.f })
  {
    VRM_ASSERT_MSG(slot < s_MaxColorAttachments, "Slot is too high: {}, max is {}", slot, s_MaxColorAttachments - 1);
    VRM_ASSERT_MSG(!isColorAttachmentUsed(slot), "Color attachment on slot {} is already used", slot);
    const Texture::Desc& desc = arrayColorTexture.getDescription();
    VRM_ASSERT_MSG(desc.sampleCount == m_samples,
                   "Samples must match: framebuffer is set with {} samples, but texture has {}", m_samples,
                   desc.sampleCount);
  ...
  inline void setDepthAttachment(const Texture& arrayDepthTexture, GLuint layer, GLint mipLevel = 0,
                                 float depthClearValue = 1.f)
  {
    ...
    glFramebufferTextureLayer(GL_DRAW_FRAMEBUFFER, GL_DEPTH_ATTACHMENT, arrayDepthTexture.getRenderId(), mipLevel,
                              layer);
  ...
  inline static void Blit(const FrameBuffer& dest, const FrameBuffer& src)
  {
    ...
    glBlitFramebuffer(0, 0, src.m_width, src.m_height, 0, 0, dest.m_width, dest.m_height, GL_DEPTH_BUFFER_BIT,
                      GL_NEAREST);
    ...
    for (size_t i = 0; i < s_MaxColorAttachments; ++i)
    {
      if (dest.m_drawBuffers.at(i) != GL_NONE && src.m_drawBuffers.at(i) != GL_NONE)
      {
        ...
        glBlitFramebuffer(0, 0, src.m_width, src.m_height, 0, 0, dest.m_width, dest.m_height, GL_COLOR_BUFFER_BIT,
                          GL_NEAREST);
      }
    }
  }
