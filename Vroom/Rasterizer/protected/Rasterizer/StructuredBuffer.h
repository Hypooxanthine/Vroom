 ...
 *
 */
class VRM_RASTERIZER_API StructuredBuffer
{
public:
  ...
  template <typename T>
  inline void setAttribute(std::span<T const> data, const SSBO430Layout::Attrib& attribute, size_t arrayIndex);

  template <typename T>
  inline void setAttribute(const T& data, const SSBO430Layout::Attrib& attribute, size_t arrayIndex);

  inline const SSBO430Layout& getLayout() const
  {
    return m_layout;
  }
  ...
  {
    return m_data.size() * sizeof(std::byte);
  }

private:

  void _setAttributeData(std::span<std::byte const> data, const SSBO430Layout::Attrib& attribute, size_t arrayIndex);
...
template <typename T>
inline void StructuredBuffer::setAttribute(const T& data, const SSBO430Layout::Attrib& attribute, size_t arrayIndex)
{
  std::byte const* dataPtr = reinterpret_cast<std::byte const*>(&data);
  _setAttributeData(std::span<std::byte const>(dataPtr, sizeof(T)), attribute, arrayIndex);
}

template <typename T>
inline void StructuredBuffer::setAttribute(std::span<T const> data, const SSBO430Layout::Attrib& attribute,
                                           size_t arrayIndex)
{
  std::byte const* dataPtr = reinterpret_cast<std::byte const*>(data.data());
  _setAttributeData(std::span<std::byte const>(dataPtr, data.size_bytes()), attribute, arrayIndex);
}
