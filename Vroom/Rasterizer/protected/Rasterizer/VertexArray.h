...
class VRM_RASTERIZER_API VertexArray
{
public:
...
public:
  ...
  inline bool isAttributeIndexEnabled(uint32_t attribIndex) const
  {
    return m_attribs.at(attribIndex).enabled;
  }

  static void        Bind(GLuint renderId);
  inline static void Bind(const VertexArray& vertexArray)
  {
    Bind(vertexArray.getRenderId());
  }
