...
/**
 * @brief Layer class. Layers are used to separate different parts of the application. Layers are rendered in the order
 * they are added, and updated in the reverse order.
 *
 */
class VRM_APPLICATION_API Layer
{
public:
...
public:
   ...
   */
  inline void setShouldHandleEvents(bool shouldHandleEvents)
  {
    m_ShouldHandleEvents = shouldHandleEvents;
  }
   ...
   */
  inline bool isHandlingEvents() const
  {
    return m_ShouldHandleEvents;
  }

  void pushRoutine(EFrameLocation::Type location, const DeferredRoutine& routine);

  inline void pushFrameEndRoutine(const DeferredRoutine& routine)
  {
    pushRoutine(EFrameLocation::ePostEndFrame, routine);
  }
