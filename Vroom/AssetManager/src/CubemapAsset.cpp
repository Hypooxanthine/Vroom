...
bool CubemapAsset::loadImpl(const std::filesystem::path& filePath)
{
  ...
  {
    VRM_LOG_ERROR("Could not parse {}. Error: {}", filePath.string(), e.what());
    return false;
  }
  ...
  VRM_LOG_INFO("Skybox loaded: {}", filePath.string());

  return true;
}

bool CubemapAsset::_loadTexture(const std::filesystem::path& filePath, ByteTextureData& textureData)
{
...
{
  VRM_CHECK_RET_FALSE_MSG(m_data.posx.getWidth() == tex.getWidth() && m_data.posx.getHeight() == tex.getHeight(),
                          "Texture sizes don't match");
...
bool CubemapAsset::_buildGpuCubemap()
{
  ...
  glTexSubImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_X, 0, 0, 0, desc.width, desc.height, desc.format, GL_UNSIGNED_BYTE,
                  m_data.posx.getData());
  glTexSubImage2D(GL_TEXTURE_CUBE_MAP_NEGATIVE_X, 0, 0, 0, desc.width, desc.height, desc.format, GL_UNSIGNED_BYTE,
                  m_data.negx.getData());
  glTexSubImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_Y, 0, 0, 0, desc.width, desc.height, desc.format, GL_UNSIGNED_BYTE,
                  m_data.posy.getData());
  glTexSubImage2D(GL_TEXTURE_CUBE_MAP_NEGATIVE_Y, 0, 0, 0, desc.width, desc.height, desc.format, GL_UNSIGNED_BYTE,
                  m_data.negy.getData());
  glTexSubImage2D(GL_TEXTURE_CUBE_MAP_POSITIVE_Z, 0, 0, 0, desc.width, desc.height, desc.format, GL_UNSIGNED_BYTE,
                  m_data.posz.getData());
  glTexSubImage2D(GL_TEXTURE_CUBE_MAP_NEGATIVE_Z, 0, 0, 0, desc.width, desc.height, desc.format, GL_UNSIGNED_BYTE,
                  m_data.negz.getData());
  ...
  m_renderCubemap.setTexture(std::move(texture));
