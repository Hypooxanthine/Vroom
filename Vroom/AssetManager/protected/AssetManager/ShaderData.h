...
class VRM_ASSETMANAGER_API ShaderData
{
public:
  ...
  struct FullShader
  {
    ...
    std::string& operator[](const EShaderType& type)
    {
      ...
      case EShaderType::eCompute:
        return compute.sourceCode;
      case EShaderType::eVertex:
        return vertex.sourceCode;
      case EShaderType::eGeometry:
        return geometry.sourceCode;
      case EShaderType::eFragment:
        return fragment.sourceCode;
      default:
        return compute.sourceCode;
      }
    }
...
private:
  ...
  std::string combineUniformBuffers(const std::vector<UniformBuffer>& global,
                                    const std::vector<UniformBuffer>& shader) const;
  std::string combineStorageBuffers(const std::vector<StorageBuffer>& global,
                                    const std::vector<StorageBuffer>& shader) const;
