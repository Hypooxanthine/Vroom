...
public:
  ...
  inline constexpr T& at(int index)
  {
    return m_Data[index];
  }
  inline constexpr const T& at(int index) const
  {
    return m_Data[index];
  }

  inline constexpr T& at(int x, int y, int channel = 0)
  {
    return at(y * m_Width * m_Channels + x * m_Channels + channel);
  }
  inline constexpr const T& at(int x, int y, int channel = 0) const
  {
    return at(y * m_Width * m_Channels + x * m_Channels + channel);
  }
  ...
  inline constexpr T texture(float x, float y, int channel = 0) const
  {
    ...
    return static_cast<T>(c00 * invDx * invDy + c10 * dx * invDy + c01 * invDx * dy + c11 * dx * dy);
  }
  ...
  inline constexpr int getChannels() const
  {
    return m_Channels;
  }
  inline constexpr int getBpp() const
  {
    return m_Channels * sizeof(T);
  }
  inline constexpr TextureFormat getFormat() const
  {
    return ChannelsToTextureFormat(m_Channels);
  }
  ...
  inline constexpr bool isGrayscale() const
  {
    return m_Channels == 1;
  }
  inline constexpr bool isRGB() const
  {
    return m_Channels == 3;
  }
  inline constexpr bool isRGBA() const
  {
    return m_Channels == 4;
  }
...
template <>
inline unsigned char* TextureData<unsigned char>::getSTBIImage(const char* filePath, int* width, int* height,
                                                               int* channels) const
{
