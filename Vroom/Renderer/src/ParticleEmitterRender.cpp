...
void ParticleEmitterRender::rebuildMaterials(const ParticleEmitter& emitter)
{
  ...
  MaterialAsset::Handle updaterAsset =
    AssetManager::Get().tryGetAsset<MaterialAsset>("Resources/Engine/Material/UpdateParticlesMaterial.json");
...
void ParticleEmitterRender::_uploadSpawnData(const ParticleEmitter& emitter) const
{
  std::span<RawEmitterSpawnData> mapped =
    m_spawnDataBuffer.mapWriteOnly<RawEmitterSpawnData>(0, sizeof(RawEmitterSpawnData), true);
...
void ParticleEmitterRender::_uploadIndirectCommandData() const
{
  std::span<RawDrawElementsIndirectCommand> mapped =
    m_indirectCommandsBuffer.mapWriteOnly<RawDrawElementsIndirectCommand>(0, sizeof(RawDrawElementsIndirectCommand),
...
void ParticleEmitterRender::_updateParticleStates(const ParticleEmitter& emitter)
{
  ...
  glm::uint maxParticleCount =
    static_cast<glm::uint>(specs.lifeTime->getUpperBound() * specs.emitRate->getEmitRate() * 1.5f + 1.f);
  ...
  if (maxParticleCount > m_maxParticleCount)
  {
    ...
    std::span<std::byte> mapped = m_particleStatesBuffer.mapWriteOnly<std::byte>(
      m_maxParticleCount * singleStatesSize, (maxParticleCount - m_maxParticleCount) * singleStatesSize, false);
...
void ParticleEmitterRender::_executeUpdateParticles(const ParticleEmitter& emitter)
{
  ...
  updaterShader.setUniform1f("u_deltaTime", DeltaTime::Get().seconds());
...

  {
    ...
    const auto     nanosecs = std::chrono::duration_cast<std::chrono::nanoseconds>(now.time_since_epoch()).count();
    const uint32_t nanosecsUint32 = static_cast<uint32_t>(nanosecs % std::numeric_limits<decltype(nanosecs)>::max());
...
void ParticleEmitterRender::_executeRenderParticles(const RenderPassContext& ctx, const glm::mat4* model)
{
  ...
  const render::Mesh& mesh   = m_mesh->getSubMeshes().at(0).renderMesh;
