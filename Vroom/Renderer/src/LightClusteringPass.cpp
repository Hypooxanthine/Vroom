...
void LightClusteringPass::onInit()
{
  ...
  m_setupClustersMat = &getPassMaterial(
    AssetManager::Get().getAsset<MaterialAsset>("Resources/Engine/Material/BuildClustersMaterial.json"));
  m_cullLightsMat =
    &getPassMaterial(AssetManager::Get().getAsset<MaterialAsset>("Resources/Engine/Material/CullLightsMaterial.json"));
...
void LightClusteringPass::onSetup(const RenderPassContext& ctx)
{
  ...
  for (size_t i = 0; i < ctx.views.size(); ++i)
  {
    ...
    setupClustersShader.setStorageBuffer("ClusterInfoBlock", m_clustersBuffer.getBuffer(), sizePerView * i,
                                         sizePerView);
