...
class VRM_RENDERER_API RenderLayout
{
public:
...
public:
  ...
  inline float getWidth(size_t col) const
  {
    return m_widths.at(col);
  }
  inline float getHeight(size_t row) const
  {
    return m_heights.at(row);
  }
  inline const render::NormalizedView& getView(size_t row, size_t col) const
  {
    return m_views.at(getViewIndex(row, col));
  }
...
private:
  ...
  inline render::NormalizedView& _getView(size_t row, size_t col)
  {
    return m_views.at(getViewIndex(row, col));
  }
