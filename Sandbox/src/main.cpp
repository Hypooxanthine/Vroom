...
protected:
  ...
  void onInit() override
  {
    ...
    for (uint8_t i = 0; i < 10; i++)
    {
      ...
      auto& plc    = entity.addComponent<vrm::PointLightComponent>();
      plc.setColor(glm::vec3{ std::cos(i * 2.f * 3.14159f / 10), 1.f, std::sin(i * 2.f * 3.14159f / 10) });
      plc.setIntensity(50.f);
      plc.setRadius(15.f);
    ...
    // Maybe another event type, which will give a scalar depending on the input (moveForward in [-1, 1] for example,
    // controlled with any input we want).
    gameLayer.getTrigger("MoveForward")
      .bindCallback(
        [this](bool triggered)
        {
          forwardValue += triggered ? 1.f : -1.f;
        });
    ...
    gameLayer.getTrigger("MoveUp").bindCallback(
      [this](bool triggered)
      {
        upValue += triggered ? 1.f : -1.f;
      });
...
private:
  ...
  vrm::FirstPersonCamera myCamera{
    0.1f, 100.f, glm::radians(90.f), 600.f / 400.f, glm::vec3{ 0.f, 0.f, 0.f },
...
int main(int argc, char** argv)
{
  ...
  gameLayer.createTrigger("MoveForward").bindInput(vrm::KeyCode::W);
  gameLayer.createTrigger("MoveBackward").bindInput(vrm::KeyCode::S);
  gameLayer.createTrigger("MoveLeft").bindInput(vrm::KeyCode::A);
  gameLayer.createTrigger("MoveRight").bindInput(vrm::KeyCode::D);
  gameLayer.createTrigger("MoveUp").bindInput(vrm::KeyCode::Space);
  gameLayer.createTrigger("MoveDown").bindInput(vrm::KeyCode::LeftShift);
  ...
  gameLayer.createCustomEvent("Exit")
    ...
    .bindCallback(
      [&app](const vrm::Event&)
      {
        VRM_LOG_INFO("Application exit has been requested by user.");
        app.exit();
      });
  gameLayer.createCustomEvent("MouseMoved").bindInput(vrm::Event::Type::MouseMoved);
